				...
				<div class="lyear-layout-sidebar-scroll">
					...
					<nav class="sidebar-main">
						<ul class="nav nav-drawer">
							<li class="nav-item"><a href="${pageContext.request.contextPath }/teacher/toindex"
							...
							<li class="nav-item nav-item-has-subnav active open"><a
								...
								<ul class="nav nav-subnav">
									...
									<li class="active"> <a href="${pageContext.request.contextPath }/teacher/toSelecttitlelist">查看选题学生</a> </li>
								</ul></li>
...
<div>
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-toolbar clearfix">
								<form id="form_query"
									action="${pageContext.request.contextPath }/teacher/toSelecttitlelist"
										...
										<tbody>
											<c:forEach var="title" items="${pageInfo.list }"
												...
												<tr>	
													...
													<td>${title.seltitlState }</td>
													<td><input type="button" class="btn btn-success btn-xs" id="agree"
																	name="agree" value='同意' onclick="agreeSelTitle('${title.sId }',${title.titlId})">
																	<input type="button" class="btn btn-danger btn-xs" id="refuse"
																	name="refuse" value='拒绝' onclick="refuseSelTitle('${title.sId }',${title.titlId})"></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
	...
	function jumpPage(pageNumber){
	  ...
	  function getStudent(sId) {
		  $.ajax({
		        type:"get",
		        url:"<%=basePath%>teacher/getStudentInfoById",
				data : {
					"sId" : sId
				},
	  ...
	  //同意学生选题
	  function agreeSelTitle(sId,titlId) {
		  if(confirm('确定同意该学生吗? 其他学生将无法选择该门课题')){
			  $.post("<%=basePath%>teacher/editSelTitle",{"sId":sId, "titlId":titlId, "seltitlState":"同意"},
						function(data){
						            if(data =="OK"){
						                alert("操作成功！");
	  ...
	  //拒绝学生选题
	  function refuseSelTitle(sId,titlId) {
		  if(confirm('确定拒绝该学生吗? 他将被限制再选择该门课题')){
			  $.post("<%=basePath%>teacher/editSelTitle",{"sId":sId, "titlId":titlId, "seltitlState":"拒绝"},
						function(data){
						            if(data =="OK"){
						                alert("操作成功！");
						                window.location.reload();
	  ...
	  function deleteTitle(titlId) {
		  if(confirm('确实要删除该课题吗? 该题的学生选题信息也会被删除')) {
				$.post("<%=basePath%>teacher/titledelete",{"titlId":titlId},
				function(data){
				            if(data =="OK"){
				                alert("课题删除成功！");
				                window.location.reload();
	    ...
	    function getTitle(titlId) {
	    	$.ajax({
		        type:"get",
		        url:"<%=basePath%>student/getTitleInfoById",
				data : {
					"titlId" : titlId
				},
	    ...
	    //申请新课题
	    function newTitle() {
	    	$.post("<%=basePath%>teacher/createTitle",
