...
<body>
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-toolbar clearfix">
								<form id="form_query"
									action="${pageContext.request.contextPath }/admin/tomajorlist?deptId=${deptId }"
							...
							<div class="card-body">
							    <a href="${pageContext.request.contextPath }/admin/todeptlist"><button type="button" class="btn btn-w-xs btn-secondary ">返回</button></a>
	...
	function jumpPage(pageNumber){
	  ...
	  function createMajor() {
		  ...
		  $.post("<%=basePath%>admin/createMajor",
	  ...
	  function editMajor(majorId) {
		    $.ajax({
		        type:"get",
		        url:"<%=basePath%>admin/getMajorById",
	  ...
	  function saveMajorinfo() {
		  ...
		  $.post("<%=basePath%>admin/updateMajor",$("#edit_major_form").serialize(),function(data){
				if(data =="OK"){
					alert("专业信息更新成功！");
					window.location.reload();
				}else{
	  ...
	  function deleteMajor(majorId,s) {
		  if(confirm('确实要删除/恢复该专业吗?')){
			  $.post("<%=basePath%>admin/deleteMajor",{"majorId":majorId,"s":s},function(data){
					if(data =="OK"){
						alert("操作成功！");
						window.location.reload();
					}else{
