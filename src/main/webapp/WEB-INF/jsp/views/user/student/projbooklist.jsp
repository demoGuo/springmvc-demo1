				...
				<div class="lyear-layout-sidebar-scroll">
					...
					<nav class="sidebar-main">
						<ul class="nav nav-drawer">
							<li class="nav-item"><a href="${pageContext.request.contextPath }/student/toindex"
							...
							<li class="nav-item nav-item-has-subnav"><a
								...
								<ul class="nav nav-subnav">
									<li><a
										href="${pageContext.request.contextPath }/student/totitlelist">学生选题</a>
									</li>
									<li><a
										href="${pageContext.request.contextPath }/student/selTitleById">我的选题</a></li>
								</ul></li>
							<li class="nav-item nav-item-has-subnav active open"><a
								href="javascript:void(0)"><i class="mdi mdi-file-outline"></i>
									技术文档管理</a>
								<ul class="nav nav-subnav">
									<li class="active"><a href="${pageContext.request.contextPath }/student/projBooklist">技术文档</a></li>
								</ul></li>
								<li class="nav-item nav-item-has-subnav"><a
								href="javascript:void(0)"><i class="mdi mdi-file-outline"></i>
									程序代码管理</a>
								<ul class="nav nav-subnav">
									<li><a href="${pageContext.request.contextPath }/student/openReportlist">程序代码 (学生)</a></li>
								</ul></li>
							...
							<li class="nav-item nav-item-has-subnav"><a
								href="javascript:void(0)"><i class="mdi mdi-attachment"></i>
									附件</a>
								<ul class="nav nav-subnav">
									<li><a href="${pageContext.request.contextPath }/student/thesisAttachmentlist">附件上传 (学生)</a></li>
								</ul></li>
							...
							<li class="nav-item nav-item-has-subnav"><a
								href="javascript:void(0)"><i class="mdi mdi-scale-balance"></i>
									总评</a>
								<ul class="nav nav-subnav">
									<li><a href="${pageContext.request.contextPath }/student/studentScore">成绩总评 (学生)</a></li>
								</ul></li>
						</ul>
					</nav>
				...
				<nav class="navbar navbar-default">
					<div class="topbar">
						...
						<div class="topbar-left">
							...
							<span class="navbar-page-title"> 提交技术文档 </span>
						</div>
						...
						<ul class="topbar-right">
							<li class="dropdown dropdown-profile"><a
								...
								<ul class="dropdown-menu dropdown-menu-right">
									<li><a
										href="${pageContext.request.contextPath }/student/topersonInfo"><i
											class="mdi mdi-account"></i> 个人信息</a></li>
									<li><a
										href="${pageContext.request.contextPath }/student/toeditPwd"><i
					...
					<div class="col-lg-12">
						<div class="card">
							<div class="card-toolbar clearfix">
								<form id="form_query"
									action="${pageContext.request.contextPath }/student/projBooklist"
									...
									<div class="input-group">
										...
										<input type="text" class="form-control"
											value="${projBook.fName}" name="fName"
											placeholder="请输入文件名称">
									</div>
								</form>
							</div>
							...
							<c:if test="${i eq 1 && i2 eq 0}">
							    <form enctype="multipart/form-data" name="f1" id="f1" action="${pageContext.request.contextPath }/file/fileUpload" method="post" onsubmit="return check()">
							    <input id="fType" type="text" name="fType" hidden="hidden" value="技术文档">
								...
								<div class="table-responsive">
									<table class="table table-bordered" id="tbodyID">
										...
										<tbody>
											<c:forEach var="projBook" items="${pageInfo.list }"
												varStatus="state">
												<tr>
													<td>${ state.index + 1}</td>
													<td>${projBook.titlName}</td>
													<td>${projBook.major}</td>
													<td>${projBook.batch}</td>
													<td>
													${projBook.fName }
													</td>
													<td><fmt:formatDate value="${projBook.uploadDatetime }" pattern="yyyy-MM-dd hh:mm:ss"></fmt:formatDate></td>
													<td>${projBook.tName }</td>
													<td><span <c:if test="${projBook.agree == '不通过'}">class="label label-danger"</c:if> <c:if test="${projBook.agree == '通过'}">class="label label-success"</c:if> <c:if test="${projBook.agree == '待通过'}">class="label label-warning"</c:if>>${projBook.agree }</span></td>
													<td>
													<input data-toggle="modal"
														data-target="#commentsInfoModal" type="button" class="btn btn-primary btn-xs" id="comment"
																	name="search2" value='查看指导教师建议' onclick="getcomment(${projBook.fId})">
													<a href="${pageContext.request.contextPath }/file/filedown?fId=${projBook.fId}"><input type="button" class="btn btn-primary btn-xs" id="download"
						...
						<div class="modal-body">
							<form id="selectById_title_form">
										<div class="form-group">
											<label for="message-text" class="control-label">技术文档建议内容：</label>
	...
	function jumpPage(pageNumber){
	  ...
	  function getfId (fId) {
	    ...
	    function getcomment(fId) {
	    	$.ajax({
		        type:"get",
		        url:"<%=basePath%>comments/getcommentsById",
		        data:{"fId":fId},
